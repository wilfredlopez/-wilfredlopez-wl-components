...
export const sanitizeDOMString = (
  untrustedString: WlSafeString | string | undefined
): string | undefined => {
  try {
    if (untrustedString instanceof WlSafeString) {
      return untrustedString.value;
    }
    if (
      !isSanitizerEnabled() ||
      typeof untrustedString !== "string" ||
      untrustedString === ""
    ) {
...
const isSanitizerEnabled = (): boolean => {
  ...
  if (config) {
    if (config.get) {
      ...
      return (
        config.sanitizerEnabled === true ||
        config.sanitizerEnabled === undefined
      );
    }
  }
  return true;
