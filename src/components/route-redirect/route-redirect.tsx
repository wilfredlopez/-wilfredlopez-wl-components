...
@Component({
  tag: "wl-route-redirect",
})
export class WlRouteRedirect implements ComponentInterface {
  /**
   ...
   * When the defined `wl-route-redirect` rule matches, the router will redirect to the path
   * specified in this property.
   *
   * The value of this property is always an absolute path inside the scope of routes defined in
   * `wl-router` it can't be used with another router or to perform a redirection to a different domain.
   *
   * Note that this is a virtual redirect, it will not cause a real browser refresh, again, it's
   * a redirect inside the context of wl-router.
   *
   ...
   *
   * `wl-router` captures this event in order to update his internal registry of router rules.
   */
  @Event() wlRouteRedirectChanged!: EventEmitter;
