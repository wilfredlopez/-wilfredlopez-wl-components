...
export class WlRouter implements ComponentInterface {
   ...
   */
  @Event() wlRouteWillChange!: EventEmitter<RouterEventDetail>;

  /**
   * Emitted when the route had changed
   */
  @Event() wlRouteDidChange!: EventEmitter<RouterEventDetail>;
  ...
  componentDidLoad() {
    window.addEventListener(
      "wlRouteRedirectChanged",
      debounce(this.onRedirectChanged.bind(this), 10)
    );
    window.addEventListener(
      "wlRouteDataChanged",
      debounce(this.onRoutesChanged.bind(this), 100)
    );
  }
  ...
  @Listen("wlBackButton", { target: "document" })
  ...
  async navChanged(direction: RouterDirection): Promise<boolean> {
    ...
    if (!path) {
      console.warn(
        "[wl-router] router could not match path because some required param is missing"
      );
      return false;
    }
  ...
  private historyDirection() {
    ...
    if (win.history.state === null) {
      ...
      win.history.replaceState(
        this.state,
        win.document.title,
        win.document.location && win.document.location.href
      );
    }
  ...
  private async safeWriteNavState(
    ...
    let changed = false;
    try {
      changed = await this.writeNavState(
        node,
        chain,
        direction,
        path,
