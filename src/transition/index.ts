...
import { NavDirection, NavOptions } from "../interfaces/Nav";
...
const getAnimationBuilder = async (
  opts: TransitionOptions
): Promise<AnimationBuilder | undefined> => {
...
export const getWlPageElement = (element: HTMLElement) => {
  if (element.classList.contains("wl-page")) {
    return element;
  }

  const wlPage = element.querySelector(
